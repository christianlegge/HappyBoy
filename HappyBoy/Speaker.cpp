...
void audio_callback(void* userdata, Uint8* stream, int len)
{
	...
	for (int i = 0; i < len / 2; i++) {
		((Uint16*)stream)[i] = (channel1.get_sample(s->global_time) + channel2.get_sample(s->global_time) + (s->apu->NR52.sound3_on ? channel3.get_sample(s->global_time, s->apu->waveram) : 0) + channel4.get_sample(s->global_time)) * 4;
